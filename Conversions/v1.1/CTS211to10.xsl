...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- =========================================
        ...
        Issue #156 - derivation is an optional parameter in an entry
        Remove if present, as it isn't there in 1.1
        See issue #120 for complete solution
        ========================================== -->


    <!-- =========================================
        Issue #119 - associationID should optional in Association
        Issue #157 - derivation should be optional w/ no default
        If absent, add it with a generated value.  This impacts Association, AssociationMsg and AssociationList
        Have to add derivation back if not present
        ========================================== -->
        ...
        <xsl:element name="{local-name()}" namespace="{$nsmap/association}">
            ...
            <xsl:if test="not(@associationID)">
                <xsl:attribute name="associationID">
                    <!-- Note: actual implementations will probably want to assign a guid or similar identifier here -->
                    <xsl:value-of select="concat(association1.1:subject/@uri,'/',generate-id())"/>
                </xsl:attribute>
            </xsl:if>
    ...
    <!-- =========================================
        ...
        Remove depth indicator working backwards
        ========================================== -->
    ...
    <!-- =========================================
        ...
        Have to strip from 1.1 if present
        ========================================== -->
    ...
    <xsl:template match="core1.1:knownEntityDescription">
        <xsl:element name="{local-name()}" namespace="{$nsmap/core}">
            <xsl:apply-templates select="node() | @*[name()!='codeSystemRole'] | processing-instruction() | comment()"/>
        </xsl:element>
    </xsl:template>
    <xsl:template
    ...
    <!-- =========================================
        ...
        Need to remove current resolution node
        ========================================== -->
    ...
    <!-- =========================================
        ...
        Remove parents link if it exists
        ========================================== -->
    ...
    <!-- =========================================
        ...
        Need to add a document URI if one isn't present
        ========================================== -->
    <!-- Covered under issue #140 below -->
    ...
    <!-- =========================================
        ...
        ========================================== -->
    <!-- This is not an issue in the XML Schema - just the model -->


    <!-- =========================================
        Issue #156 -AssociationDirectory needs an optional derivation.
        Have to strip it when going back to 1.0
        ========================================== -->
    <!-- covered as part of issue #120 -->

    <!-- =========================================
        Issue #157 - Association derivation should be optional and not have a default
        If derivation is missing, have to supply the default value (ASSERTED) going back to 1.0
        ========================================== -->
    <!-- covered as part of issue #119 -->
